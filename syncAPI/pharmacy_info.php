...
$syncArray[0]["errorMsg"] = "약국 정보 동기화 시작";
$syncArray[0]["sdate"] = $sdate;
$syncArray[0]["edate"] = $sdate;
$syncArray[0]["syncID"] = "PHARMACY_INFO";
$_pdoObject->update_sync_monitoring_info($syncArray[0]);

$_pageNo = 1;
$_numOfRows = 2000;
$_TPage = 0;
$_ip = 'http://apis.data.go.kr/B552657/ErmctInsttInfoInqireService/getParmacyFullDown?serviceKey='._SERVICE_KEY.'&numOfRows='.$_numOfRows.'&pageNo='.$_pageNo;
...
if((string)$xml->header->resultCode == "00")
{
	...
	for($i=0; $i<$cnt; $i++) 
	{
		...
		if($_pdoObject->update_pharmacy_info($data) == true)
		{
			$_tSuccess++;
		}
		else
	...
	if($_pageNo < $_TPage)
	{
		...
		{
			$_ip = 'http://apis.data.go.kr/B552657/ErmctInsttInfoInqireService/getParmacyFullDown?serviceKey='._SERVICE_KEY.'&numOfRows='.$_numOfRows.'&pageNo='.$p;
			$xml = simplexml_load_file($_ip);
			if((string)$xml->header->resultCode == "00")
			{
				$cnt = count($xml->body->items->item);
	...
	$syncArray[0]["tSuccess"] = $_tSuccess;
	$syncArray[0]["tFail"] = $_tFail;
	$syncArray[0]["errorMsg"] = "약국 정보 동기화 완료";
...
else
{
    ...
    $syncArray[0]["errorMsg"] = "약국 정보 동기화 실패 [".(string)$xml->header->resultMsg."]";
