...
if((int)date("H",time()) == 1) {
    try
    {
        $deleteSql = "DELETE FROM KMA_WEATHER_DAY_INFO WHERE `adate` < DATE_ADD(NOW(), INTERVAL '-2' DAY) ";
    ...
    try
    {
        $deleteSql = "DELETE FROM KMA_WEATHER_NEW_DAY_INFO WHERE `adate` < DATE_ADD(NOW(), INTERVAL '-2' DAY) ";
    	$stmt = $_pdoObject->_connection->prepare($deleteSql);
    	$_pdoObject->_connection->beginTransaction();
		$stmt->execute();
		$_pdoObject->_connection->commit();
    ...
    try
    {
        $deleteSql = "DELETE FROM KMA_API_INFO WHERE `adate` < DATE_ADD(NOW(), INTERVAL '-2' DAY) ";
    	$stmt = $_pdoObject->_connection->prepare($deleteSql);
    	$_pdoObject->_connection->beginTransaction();
		$stmt->execute();
		$_pdoObject->_connection->commit();
...
} else if((int)date("H",time()) == 0) {
    try
    {
        $sql = "UPDATE KMA_WEATHER_DAY_INFO AA 
                JOIN ( SELECT * FROM KMA_WEATHER_NEW_DAY_INFO WHERE adate = '".date("Y-m-d", strtotime("-1 days"))."' ) BB 
                ON AA.basedate = BB.basedate AND AA.nx = BB.nx AND AA.ny = BB.ny SET 
                AA.RN1 = BB.RN1,
                AA.PTY = BB.PTY,
                AA.UUU = BB.UUU,
                AA.VVV = BB.VVV,
    ...
    {
        $_pdoObject->_connection->rollback();
    	$_pdoObject->error("Query error : [".$sql."] ".$e->getMessage()."\n");
    }
}
...
if(count($locationArray) > 0) {
    for($i=0; $i<count($locationArray); $i++) {
        $_syncTitleName = $locationArray[$i]["sido"]." 초단기예보 날씨 정보 동기화 ";                
        $_tSuccess = 0;
    	$_tFail = 0;
    	...
    	$syncArray[0]["syncID"] = "KMA_WEATHER_DAY_".$locationArray[$i]["areaNo"];
    	$_pdoObject->update_sync_monitoring_info($syncArray[0]);
        
        // $ch = curl_init();
        $url = 'http:///localhost/syncAPI/kma_weather_day_info.php';
        ...
        /* ==================================================================== */
        $_syncTitleName = $locationArray[$i]["sido"]." 초단기실황 날씨 정보 동기화 ";                
        $_tSuccess = 0;
    	$_tFail = 0;
    	$_totalCount = (int)$locationArray[$i]["cnt"];
    	$sdate = date("Y-m-d H:i:s", time());
    	...
    	$syncArray[0]["sdate"] = $sdate;
    	$syncArray[0]["edate"] = $sdate;
    	$syncArray[0]["syncID"] = "KMA_WEATHER_NEW_DAY_".$locationArray[$i]["areaNo"];
    	$_pdoObject->update_sync_monitoring_info($syncArray[0]);
        
        // $ch = curl_init();
        $url = 'http:///localhost/syncAPI/kma_weather_day_new_info.php';
        $queryParams  = '?sido=' . urlencode($locationArray[$i]["sido"]);
        $queryParams .= '&areaNo=' . $locationArray[$i]["areaNo"];
