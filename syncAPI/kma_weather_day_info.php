...
// https://appsstudio.site/syncAPI/kma_weather_day_new_info.php?sido=서울특별시&areaNo=1100000000
...
if(count($locationArray) > 0) {
    $_syncTitleName = $sido." 초단기예보 날씨 정보 동기화 ";               
    $_tSuccess = 0;
	$_tFail = 0;
	...
	$syncArray[0]["syncID"] = "KMA_WEATHER_DAY_".$areaNo;
    ...
    for($k=0; $k<count($locationArray); $k++) {                    
    	...
    	$_ip = "http://apis.data.go.kr/1360000/VilageFcstInfoService/getUltraSrtFcst";
        $_ip .= "?serviceKey="._SERVICE_KEY_NEW;
        $_ip .= "&base_date=".date("Ymd", time());
        $_ip .= "&base_time=".date("H", time())."30";
            ...
            for($i=0; $i<count($xml->body->items->item); $i++) 
        	{
                ...
                if($i == 0) {
                   ...
                   if($fcstDateTime != $data["basedate"]) {
                        if($_pdoObject->update_kma_weather_day_info($data) == false)
                		{
                ...
                if($item->category == "RN1") { $data["RN1"] = $item->fcstValue; }
                if($item->category == "PTY") { $data["PTY"] = $item->fcstValue; }
                if($item->category == "UUU") { $data["UUU"] = $item->fcstValue; }
                if($item->category == "VVV") { $data["VVV"] = $item->fcstValue; }
                if($item->category == "REH") { $data["REH"] = $item->fcstValue; }
                if($item->category == "SKY") { $data["SKY"] = $item->fcstValue; }
                if($item->category == "T1H") { $data["T1H"] = $item->fcstValue; }
                if($item->category == "VEC") { $data["VEC"] = $item->fcstValue; }
                if($item->category == "WSD") { $data["WSD"] = $item->fcstValue; }
                if($item->category == "LGT") { $data["LGT"] = $item->fcstValue; }
        	}
        	
        	if($data["SKY"] != "" && $data["PTY"] != "") 
        	{
            	if($_pdoObject->update_kma_weather_day_info($data) == true)
        		{
        			$_tSuccess++;
        		}
        		else
